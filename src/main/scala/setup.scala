...
object setup {
  ...
  implicit val getComputer = GetResult(r => Computer(r.nextInt, r.nextString, r.nextInt))
  implicit val getCompany = GetResult(r => Company(r.nextInt, r.nextString))

  def initDb(implicit session: Session) = {
    try { Companies.ddl.drop } catch { case e: SQLException => }
    try { Computers.ddl.drop } catch { case e: SQLException => }
    ...
    Seq(
      ...
      Company((43), "Samsung Electronics")).foreach(Companies.+=)
    ...
    Seq(
      ...
      Computer((574), "iPhone 4S", (1))).foreach(Computers.+=)
  }
}
